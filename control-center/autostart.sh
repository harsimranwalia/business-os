...
#
# This script only ever manages the local server on :6666, gated by
...
# start.sh exits 0 when something is already listening on :6666, and Ctrl-C in
...
if [ "$HOST_KIND" = mac ]; then
  ...
  # longer contend for a port either: life-os keeps :7777 and this one moved to
  # :6666 on 2026-08-31, because both had hardcoded 7777 and life-os's is a
  # KeepAlive agent — so whichever lost the race respawned into a port it could
  # never win, and business-os was the one that kept losing.
  plist() {
    cat <<PLIST
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Triggers>
    <!-- Scoped to this user, not "any user": the server reads this repo's .env
         ...
         at :6666 half a minute after signing in, and the shell PATH, the
