...
created: 2026-08-31
adrs: [ADR-005]
<!-- merge note: local claimed created 2026-08-29 / adrs: [], remote claimed created 2026-08-31 / adrs: [ADR-005] (more recent), kept remote -->
one_way_doors: []
touches_data: false
...
An earlier pass (2026-08-29) proposed a single-action design instead of the
two-action split above: extend `url-shortener`'s new
`get_or_create_restaurant_qr` action to also return the restaurant identity
fields (`website`, `logo_url`) needed by the ported generator components,
rather than adding a separate `brand-portal` action for that read. Rationale
kept for reference:

- The action already reads the `restaurants` row to compute `destination_url`
  and the row's `name`; returning `website`/`logo_url` alongside was framed as
  the same query, not a second one, avoiding a widened `RestaurantContext`
  fetch used on every page.
- Proposed collapsing AC1 (view existing) and AC2 (generate if missing) into
  one find-or-create action returning `created: boolean`, so the UI could
  distinguish "already had one" from "just made one" without extra client
  logic.
- Considered and rejected putting the new action in `brand-portal` instead of
  `url-shortener`, on the grounds that `brand-portal` doesn't touch
  `shortened_urls` today and adding QR generation there would either need a
  new `_shared/qrCode.ts` or duplicate `generateShortCode`/`generateQRCode`.
  (The later pass resolves this by keeping QR generation in `url-shortener`
  and only adding the restaurant-identity *read* to `brand-portal`, avoiding
  the duplication concern while still splitting concerns across the two
  functions.)
- Flagged the same pre-existing risks later confirmed in the newer pass: the
  three independent copies of "list, filter, create if absent" logic in
  `aiorders-admin-hub` that must not be ported unchanged (403 for
  restaurant-manager users); `_shared/restaurantAccess.ts`'s two known-broken
  call sites in `brand-portal`'s `website.ts`/`feedback.ts`/`offers.ts`;
  `shortened_urls` having no tracked migration or confirmed uniqueness
  constraint, so a race could produce duplicate rows for the same
  restaurant + `qr_type` (pre-existing, not introduced by either pass).
